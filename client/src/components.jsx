      ...
      <div className="modal-body">
        ...
        <div className="preview-panel">
          ...
          <div className="preview-grid">
            ...
            <Metric label="Real no Take" value={preview?.realTake} />
            <Metric label="Real no Stop" value={preview?.realStop} />
          </div>
        </div>
        {error && <div className="error-box">{error}</div>}
      </div>
...
export function Metric({ label, value, sub, kind, icon }) {
  return <div className={`metric ${kind || tone(value)}`}>{icon && <span className="metric-icon">{icon}</span>}<span>{label}</span><strong>{value === undefined ? '—' : money(value)}</strong>{sub && <small>{sub}</small>}</div>;
}

const targetOptions = [
  ['mesaTake', 'Mesa Take', 'mesa'], ['mesaStop', 'Mesa Stop', 'mesa'],
  ['realTake', 'Real no Take da Mesa', 'real'], ['realStop', 'Real no Stop da Mesa', 'real'],
];
...
function CopyValue({ label, value, display, valueClass = '', compact = false }) {
  ...
  return <button type="button" className={`copy-value ${compact ? 'compact' : ''} ${copied ? 'copied' : ''}`} onClick={copy} title={`Copiar ${label}`}>
    <small>{label}</small><span className={valueClass}>{display ?? value}</span><em>{copied ? <Check size={12}/> : <Copy size={12}/>}</em>
  </button>;
}
...
export function AdjustmentModal({ operation, settings, onClose, onSaved }) {
  ...
  async function calculate(event) {
    ...
    if (realized === '') return setError('Informe o resultado efetivamente obtido.');
    try {
      const result = await api('/calculate-adjustment', { method: 'POST', body: JSON.stringify({
        market, target, realized, mode, ticks, quantity, targetField,
    ...
    {tab === 'combined' ? <CombinedAdjustment operation={operation} settings={settings} /> : <>
      <div className="modal-body adjustment-body">
        <div className="original-strip">
          <span><small>Mesa</small><strong>{operation.mesaContracts} contratos</strong></span>
          <span><small>Real</small><strong>{operation.realLots} lotes</strong></span>
        ...
        <div className="adjust-grid">
          <section className="adjust-form">
            ...
            <Field label="Qual foi o resultado efetivamente obtido?" hint="aceita positivo e negativo"><div className="money-input"><span>US$</span><input type="number" step="0.01" value={realized} onChange={e => { setRealized(e.target.value); setCalculation(null); }} placeholder="0,00" /></div></Field>
          ...
          <section className="adjust-results">
            <div className="summary-grid">
              <Metric label="Meta original" value={target} />
              <Metric label="Realizado" value={realized === '' ? undefined : realized} />
              <Metric label="Falta" value={calculation?.balance} kind="pending" />
              <Metric label="Resultado projetado" value={selected?.accumulated} kind={selected ? tone(selected.accumulated) : 'neutral'} />
            </div>
            {!calculation ? <div className="empty-result"><div><Calculator size={28}/></div><h3>Pronto para calcular</h3><p>Informe o realizado e escolha um modo para encontrar o melhor ajuste.</p></div> : calculation.suggestions.length === 0 ? <div className="empty-result"><h3>Meta já atingida</h3><p>Não há saldo pendente para ajustar.</p></div> : <div className="suggestions">
              ...
              <div className="suggestion-table">
                <div className="suggestion-row header"><span>{market === 'mesa' ? 'Contratos' : 'Lotes'}</span><span>Take</span><span>Stop</span><span>Resultado</span><span>Diferença</span><span></span></div>
              ...
              {selected && <div className="selected-summary"><span>Após o ajuste</span><strong className={tone(selected.accumulated)}>{money(selected.accumulated)}</strong><small>Meta: {money(target)} · diferença {money(selected.difference)}</small></div>}
              {selected?.impact && <div className="copy-config-card">
                ...
                <div className="copy-config-main"><CopyValue label="Mesa" value={selected.mesaContracts} display={`${selected.mesaContracts} contratos`}/><CopyValue label="Conta Real" value={selected.realLots} display={`${selected.realLots} lotes`}/><CopyValue label="Take" value={selected.takeTicks} display={`${selected.takeTicks} ticks`}/><CopyValue label="Stop proporcional" value={selected.stopTicks} display={`${selected.stopTicks} ticks`}/></div>
                <div className="copy-impact-grid"><CopyValue label="Mesa no TP" value={selected.impact.mesaTake} display={money(selected.impact.mesaTake)} valueClass={tone(selected.impact.mesaTake)}/><CopyValue label="Mesa no SL" value={selected.impact.mesaStop} display={money(selected.impact.mesaStop)} valueClass={tone(selected.impact.mesaStop)}/><CopyValue label="Real no TP" value={selected.impact.realTake} display={money(selected.impact.realTake)} valueClass={tone(selected.impact.realTake)}/><CopyValue label="Real no SL" value={selected.impact.realStop} display={money(selected.impact.realStop)} valueClass={tone(selected.impact.realStop)}/></div>
              </div>}
            </div>}
          </section>
        </div>
...
function Comparison({ title, values, subtle, highlight }) {
  return <section className={`comparison ${subtle ? 'subtle' : ''} ${highlight ? 'highlight' : ''}`}><h4>{title}</h4><div>{[['mesaTake','Mesa Take'],['mesaStop','Mesa Stop'],['realTake','Real Take'],['realStop','Real Stop']].map(([key,label]) => <span key={key}><small>{label}</small><strong className={tone(values[key])}>{money(values[key])}</strong></span>)}</div></section>;
}
...
export function HistoryPanel({ operation }) {
  ...
  return <div className="history-panel"><button onClick={toggle}><History size={15}/>{operation.adjustmentCount} {operation.adjustmentCount === 1 ? 'ajuste registrado' : 'ajustes registrados'}{open ? <ChevronDown size={16}/> : <ChevronRight size={16}/>}</button>{open && <div className="history-list">{items.map((item, index) => {
    ...
    return <article key={item.id}><div className="history-line"><span>AJUSTE #{items.length - index}</span><small>{dateTime(item.createdAt)}</small></div><div className="history-details"><span><small>Motivo</small><strong>{targetOptions.find(option => option[0] === item.targetField)?.[1]}</strong></span><span><small>Meta</small><strong className={tone(item.targetValue)}>{money(item.targetValue)}</strong></span><span><small>Realizado</small><strong className={tone(item.realizedValue)}>{money(item.realizedValue)}</strong></span><span><small>Falta</small><strong className="pending">{money(item.balance)}</strong></span><div className="history-config"><small><span>Copiar configuração</span><Copy size={11}/></small><div className="history-copy-grid"><CopyValue compact label="Mesa" value={mesaQuantity} display={`${mesaQuantity} contratos`}/><CopyValue compact label="Real" value={realQuantity} display={`${realQuantity} lotes`}/><CopyValue compact label="Take" value={takeTicks} display={`${takeTicks} ticks`}/><CopyValue compact label="Stop" value={stopTicks} display={`${stopTicks} ticks`}/></div></div><span><small>Previsto</small><strong className={tone(item.predictedResult)}>{money(item.predictedResult)}</strong></span></div></article>;
  })}</div>}</div>;
}
...
export function DashboardHeader({ user, onAdd, onSettings, onLogout, operations }) {
  ...
  return <>
    ...
    <div className="overview-cards">
      ...
      <div className="overview-card"><div className="overview-icon green"><TrendingUp size={20}/></div><span><small>Potencial Mesa</small><strong className="positive">{shortMoney(totals.take)}</strong><em>cenário de take</em></span></div>
      <div className="overview-card"><div className="overview-icon red"><TrendingDown size={20}/></div><span><small>Risco Mesa</small><strong className="negative">-{shortMoney(totals.exposure).replace('+','')}</strong><em>cenário de stop</em></span></div>
